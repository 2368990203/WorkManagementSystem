...
import team.work.core.mapper.StudentMapper;
import team.work.core.model.Student;
import team.work.core.service.IStudentService;
...
@Service
public class StudentService extends TServiceImpl<StudentMapper,Student> implements IStudentService {
   /**************************CURD begin******************************/
   // 创建
   @Override
   ...
   // 删除
   @Override
   public Boolean deleteStudent(Object ids,String reviser) {
   ...
   // 查询
   @Override
   public List<Student> findByIds(Object ids) {
   ...
   /**************************CURD end********************************/
    ...
    public Page page(int index, int pageSize, String w) {
        ...
        List<JSONObject> grades = baseMapper.getPage(w,limit,pageSize);
            ...
            return new Page(F.f2l(grades,"id","classId"), pageSize, totalCount, totalPage, currentPage);
    }

    //全查
    public List<JSONObject> queryAll(String where) {
        List<JSONObject> list = baseMapper.queryAll(where);
        return F.f2l(list,"id","classId","creator","reverse");
    }
