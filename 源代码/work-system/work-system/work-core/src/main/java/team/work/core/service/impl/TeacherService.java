...
import team.work.core.mapper.TeacherMapper;
import team.work.core.model.Teacher;
import team.work.core.service.ITeacherService;
...
@Service
public class TeacherService extends TServiceImpl<TeacherMapper,Teacher> implements ITeacherService {
   /**************************CURD begin******************************/
   // 创建
   @Override
   ...
   // 删除
   @Override
   public Boolean deleteTeacher(Object ids,String reviser) {
   ...
   // 查询
   @Override
   public List<Teacher> findByIds(Object ids) {
   ...
   /**************************CURD end********************************/
    ...
    public Page page(int index, int pageSize, String w) {
        ...
        List<JSONObject> grades = baseMapper.getPage(w,limit,pageSize);
            ...
            return new Page(F.f2l(grades,"id","academyId"), pageSize, totalCount, totalPage, currentPage);
    }

    //全查
    public List<JSONObject> queryAll(String where) {
        List<JSONObject> list = baseMapper.queryAll(where);
        return F.f2l(list,"id","academyId","creator","reverse");
    }
