...
$(function () {
    ...
    meetingList.get(index, size);
})

var meetingList = {
    get: function (index, size) {
        var param = {url: baseModule.meetingApi + '/MeetingList/' + index + '-' + size};
...
var render = {
    list: function () {
        var template = doT.template($("#meeting-list-template").text());
        $('#meeting-list').html(template(listData));
    }
};
...
var helper = {
    ...
    attendanceStatus: function (_attendanceStatus) {
        switch (parseInt(_attendanceStatus)) {
            case 1 :
                return "未签到";
            case 2 :
                return "已签到";
