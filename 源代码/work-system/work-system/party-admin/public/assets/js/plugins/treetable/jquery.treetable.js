...
jQuery.TreeTable = function (tree_id, heads, tNodes) {
    if ($("#" + tree_id).length > 0) {
        ...
        for (var i = 0, l = newNodes.length; i < l; i++) {
            ...
            body_html += "<tr style='"+style+"' data-tt-id='" + node.id + "' " + (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") + "><td class = 'td_left'  ><span class='" + (node.isParent ? "parent" : "children") + "'>" + node.name + "</span></td>";
