...
(function (factory) {
    ...
    var dom = (function () {
        ...
        return {
            ...
            isList: isList,
            isTable: isTable,
            isVideo: makePredByNodeName('VIDEO'),
            isData: isData,
            isCell: isCell,
        ...
        this.insertParagraph = function (editable) {
            ...
            if (splitRoot) {
                ...
                if (dom.isEmpty(splitRoot) && dom.isLi(splitRoot)) {
                    ...
                    nextPara = dom.splitTree(splitRoot,point);
    ...
    */
        ...
        this.insertImages = function (files) {
            $.each(files, function (idx, file) {
                ...
                if (options.maximumImageFileSize && options.maximumImageFileSize < file.size) {
                    ...
                    var upImgSuccess = function(result){
                        loading.hide();
                        var data = result.data;
                        return self.insertImage(data.source_url.replace("http:","").replace("https:",""), result.data);
                    };
                    var _file = file;
                    // var _fileName = 'course/'+string.random() +"." +getSuffix(filename);

                     var _fileName = ' partySystem/course/picture/'+string.random() +"." +getSuffix(filename);
                    cos.uploadFile(_fileName, _file, upImgSuccess);
                }
            });
        };


        this.insertVideos =  function (files) {

            $.each(files, function (idx, file) {
                var filename = file.name;
                var suf =getSuffix(filename);
                var accept=["mp4","m4v","ogg","ogv","webm"];
                var flag =false;
                for (var i =0;i<accept.length;i++){
                    if(accept[i]==suf){
                        flag=true;
                ...
                if (options.maximumVideoFileSize && options.maximumVideoFileSize < file.size) {
                    context.triggerEvent('video.upload.error', lang.video.maximumFileSizeError);
                } else {
                    var upVideoSuccess = function(result){
                        loading.hide();
                        var data = result.data;
                        if (data!=null&&data!="") {
                            var $video = $('<video id="video" name="VIDEO" controls preload="meta"data-setup="{}">')
                                //'+data.vid+
                                .attr('src', data.source_url.replace("http:","").replace("https:",""))
                                .attr('width', '640').attr('height', '360');
                            $video.addClass('note-video-clip');
                         //   range.create(editable).insertNode( $video[0]);
                         //  range.createFromNodeAfter( $video[0]).select();
                             var p = document.createElement('p');//创建p节点
                            p.innerHTML = '<br><br>';//p节点显示的文字
                            context.invoke('editor.insertNode',  $video[0]);
                            context.invoke('editor.insertNode', p);
                            afterCommand();


                            // range.create(editable).insertNode(p);
                         //   range.createFromNodeAfter(p).select();
                          //  afterCommand();
                  ...
                  //  var _fileName = 'course/'+string.random() +"." +getSuffix(filename);
                    var _fileName = ' partySystem/course/video/'+string.random() +"." +suf;

                    cos.uploadFile(_fileName, _file, upVideoSuccess);
                }
            });
        };

        /**
         * insert insertLocalVideo
         * @param {File[]} files
         */
        this.insertLocalVideo =   function (files) {
            var callbacks = options.callbacks;

            // If onImageUpload options setted
            if (callbacks.onVieoUpload) {
                context.triggerEvent('video.upload', files);
    ...
    var Clipboard = function (context) {
        ...
        this.pasteByEvent = function (event) {
            ...
            var str = $editable[0].innerHTML;
            console.log(str);
            str.replace(/<img.*src\\s*=\\s*(.*?)[^>]*?>/, function (match, capture) {
                console.log(capture);
    ...
    var Handle = function (context) {
        ...
        this.update = function (target) {
            ...
            var isVideo = dom.isVideo(target);
            ...
            if (isVideo) {
                ...
                // include margin
                var videoSize = {
                    w: $video.outerWidth(true),
                    h: $video.outerHeight(true)
                };
              ...
              //  var sizingText = videoSize.w + 'x' + videoSize.h;
              //  $selection.find('.note-control-selection-info').text(sizingText);
                context.invoke('editor.saveTarget', target);
            } else {
                this.hide();
        ...
        this.addVideoPopoverButtons = function () {
            ...
            context.memo('button.removeVideo', function () {
                return ui.button({
                    ...
                    tooltip: lang.video.remove,
                    click: context.createInvokeHandler('editor.removeVideo')
                }).render();
            });
        };
    ...
    var ImageDialog = function (context) {
        ...
        this.initialize = function () {
            ...
            var body = '<div class="form-group note-group-select-from-files">' +
                ...
                '<input class="note-image-input form-control" type="file" name="files" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg" />' +
                imageLimitation +
                '</div>';
    ...
    var VideoDialog = function (context) {
        ...
        this.initialize = function () {
            ...
            if (options.maximumVideoFileSize) {
                ...
                var readableSize = (options.maximumVideoFileSize / Math.pow(1024, unit)).toFixed(2) * 1 +
                    ' ' + ' KMGTP'[unit] + 'B';
                videoLimitation = '<small>' + lang.video.maximumFileSize + ' : ' + readableSize + '</small>';
            }


            var body =
                '<div class="form-group note-form-group note-group-select-from-files">'+
                '<label class="note-form-label">' + lang.video.selectFromFiles + '</label>'+
                '<input class="note-video-input note-form-control note-input" '+
                ' type="file" name="files" accept=".mp4,.m4v,.ogg,.ogv,.webm" multiple="multiple" />'+
                videoLimitation+'<small>'+"注：推荐上传MP4格式视频"+'</small>'+
                '</div>';
        ...
        this.createVideoNode = function (url) {
            ...
            if (ytMatch && ytMatch[1].length === 11) {
                ...
                $video = $('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>')
                    ...
                    .attr('height', '310')
                    .attr('width', '500')
                    //   http://v.qq.com/txp/iframe/player.html?vid=a00221hqpko
        ...
        this.show = function () {
            ...
            this.showVideoDialog(text).then(function (data) {
                ...
                if (typeof data === 'string'){
                    ...
                    context.invoke('editor.insertLocalVideo', data);
                }
            }).fail(function () {
                context.invoke('editor.restoreRange');
            });
        ...
        this.showVideoDialog = function (text) {
            ...
            return $.Deferred(function (deferred) {
                var $videoUrl = self.$dialog.find('.note-video-url'),
                    ...
                    $videoInput = self.$dialog.find('.note-video-input');
...
var editor = {
    ...
    set:function(_id, _html){
        $(_id).summernote('code', _html);
    },
    get:function (_id) {
        return $(_id).summernote('code');
    }
}
